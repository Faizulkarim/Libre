...
extension UserModel {
    static var currentUser : UserModel? {
        set{
            ...
            UserDefaults.standard.set(unwarrapedKey.toDictionary(), forKey: UserDefaultsKeys.kSignUpUserData.rawValue)
            UserDefaults.standard.synchronize()
            
        } get {
            
            guard let userDict = UserDefaults.standard.value(forKey: UserDefaultsKeys.kSignUpUserData.rawValue) as? [String: Any] else {
                return nil
            }
    ...
    var mobileWithContryCode: String {
        guard let user = UserModel.currentUser else { return "" }
        return JSON("+" + user.countryCode.replacingOccurrences(of: "+", with: "") + " - " + user.mobileNumber).stringValue
    }
    
    class func removeCurrentUser(){
        if UserDefaults.standard.value(forKey: UserDefaultsKeys.kSignUpUserData.rawValue) != nil {
            UserDefaults.standard.removeObject(forKey: UserDefaultsKeys.kSignUpUserData.rawValue)
            UserDefaults.standard.synchronize()
        }
    }
